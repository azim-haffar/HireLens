...
export default function Register() {
  ...
  const handleGoogle = async () => {
    ...
    catch (err) { toast.error(err.message || t('auth.googleFailed')) }
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4
                    bg-gradient-to-br from-violet-50 via-white to-brand-50
                    dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      <div className="absolute top-[-10%] left-[-5%] w-96 h-96 bg-violet-400 dark:bg-violet-800 blob pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-5%] w-80 h-80 bg-brand-400 dark:bg-brand-700 blob-2 pointer-events-none" />
      ...
      <div className="relative w-full max-w-md animate-fade-in-up">
        ...
        <div className="flex flex-wrap justify-center gap-x-4 gap-y-1 mb-6">
          {perks.map(p => (
            <span key={p} className="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
        ...
        <div className="card-glass p-8 shadow-xl">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-6">{t('auth.signUpHeading')}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" className="input pl-10" placeholder={t('auth.passwordMinPlaceholder')}
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-3 w-4 h-4 text-gray-400" />
                <input type="password" className="input pl-10" placeholder={t('auth.confirmPasswordPlaceholder')}
            ...
            <button type="submit" disabled={loading} className="btn-primary w-full py-2.5 gap-2 mt-2">
              {loading ? (
                <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />{t('auth.creatingAccount')}</>
              ) : (
